...
//        NSLog(@"没有开启声音输入回调");
    ...
    // 声音倍数调整
    XBEchoCancellation *ref = (__bridge XBEchoCancellation *)inRefCon;
    scale_volume(bufferList.mBuffers[0].mData, bufferList.mBuffers[0].mData, bufferList.mBuffers[0].mDataByteSize, 1 * ref.volumeScale);
    ...
    if (echoCancellation.bl_input2) {
        ...
        status = CMSampleBufferSetDataBufferFromAudioBufferList(buff, kCFAllocatorDefault, kCFAllocatorDefault, 0, &bufferList);
        if (status) { //失败
            return status;
        }
        echoCancellation.bl_input2(buff);
...
+ (void)scaleVolumeValue:(AudioBufferList *)bufferList scale:(float)scale {
    int in_len = bufferList->mBuffers[0].mDataByteSize;
    short *in_buf = bufferList->mBuffers[0].mData;
    short *out_buf = bufferList->mBuffers[0].mData;
    scale_volume(out_buf,in_buf,in_len,scale);
}

void scale_volume(short* out_buf,short* in_buf,int in_len, float scale) {
    int i,tmp;
