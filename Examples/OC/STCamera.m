                         ...
                         needYuvOutput:(BOOL)needYuvOutput
{
    ...
    if (self) {
        ...
        self.bufferQueue = dispatch_queue_create("com.live.STCameraBufferQueue", priorityAttribute);
        //dispatch_set_target_queue(self.bufferQueue, dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_HIGH, 0));
        ...
        if ([self.videoConnection isVideoMirroringSupported]) {
            BOOL mirrored = _mirrored ?: iDevicePosition == AVCaptureDevicePositionFront;
            [self.videoConnection setVideoMirrored:mirrored];
        }
        ...
        if (_session) {
            ...
            if (targetDevice && [self judgeCameraAuthorization]) {
                ...
                if ([_videoConnection isVideoMirroringSupported]) {
                    BOOL mirrored = _mirrored ?: devicePosition == AVCaptureDevicePositionFront;
                    [_videoConnection setVideoMirrored:mirrored];
                }
...
- (CGRect)getZoomedRectWithRect:(CGRect)rect scaleToFit:(BOOL)bScaleToFit
{
    ...
    NSLog(@"getZoomedRectWithRect %@ %@ %@", self.dataOutput.videoSettings,NSStringFromCGRect(rect),  NSStringFromCGRect(rectRet));
    return rectRet;
}
